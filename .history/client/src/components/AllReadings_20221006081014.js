...
const AllReadings = ({readings, setReadings }) => {
	...
	const renderReadings = readings.map((bgl) => {
		return(
			<Card className="bgl-card" key= {bgl.id}>
				<Card.Body className="link-text-style" as={Link} to= {`/bgls/${bgl.id}`}>
	...
	const bglPages = () => {
    let pages = [];
    let numOfPages = renderReadings.length / 10 + 1;
    ...
    for( let pageNum = 1; pageNum <= numOfPages; pageNum++ ) {
      pages.push(
        <Pagination.Item
          ...
          active={ currentRange[1] / 10 === pageNum }
          disabled={ currentRange[1] / 10 === pageNum }
        >
          { pageNum }
        </Pagination.Item>
      )
	...
	return(
		...
		{errors ? errors.map(e => <section>{e}</section>):null}		
			<Container className="all-bgl-container-style">
				...
				<Container>
					<Pagination className="pages">{ bglPages() }</Pagination>
				</Container>
